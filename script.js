...
let {arr,objHeader} = state;
const data = getData("data.json");
data.then((els) => {
  for (let i = 0; i < els.length; i++) {
    const {id,name,phone,about,eyeColor} = els[i];
    const {firstName,lastName}  = name;
    const item = {id,firstName, lastName, about, eyeColor, phone};
    arr.push(item);

  }
  let root = document.querySelector("#root"); // Получаем корень-узел #root
  createTable(objHeader,arr,root); // Создаем таблицу
